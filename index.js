...
client.once(Events.ClientReady, async () => {
  console.log('🔥 INTER MOLDS BOT IS ONLINE');
...
client.on(Events.GuildMemberAdd, async member => {
  ...
  // ⏱️ THE 5-MINUTE IDLE KILL SWITCH
  setTimeout(async () => {
    const data = onboardingData.get(member.id);
    // ONLY kick if they are still 'idle' (haven't submitted their info)
    if (data && data.status === 'idle') {
      try {
        ...
        await member.kick("Idle timeout: 5 minutes without submission").catch(() => {});
      } catch (e) { console.log("Timer Cleanup Error:", e); } finally { onboardingData.delete(member.id); }
    }
  }, 5 * 60 * 1000);
});
...
client.on(Events.InteractionCreate, async interaction => {
  try {
    ...
    if (interaction.isChatInputCommand() && interaction.commandName === "setup-broadcast") {
      ...
      await interaction.channel.send({ content: "📢 **Broadcast Panel**\nClick below to start:", components: [new ActionRowBuilder().addComponents(btn)] });
    ...
    if (interaction.isModalSubmit()) {
      ...
      if (interaction.customId === 'onboarding_modal') {
        ...
        return interaction.reply({ content: "✅ Info submitted. Admin will review shortly.", ephemeral: true });
      }
    }

    // 3. BROADCAST TARGET SELECTOR
    ...
    if (interaction.isButton()) {
      ...
      // APPROVE ACTION (NUCLEAR WIPE INCLUDED)
      if (interaction.customId.startsWith("approve_")) {
        ...
        // 🔥 THE NUCLEAR WIPE (Welcome Channel reset)
        ...
        await member.send(`✅ Welcome to **Inter Molds, Inc.** 🎉`).catch(() => {});
        await member.send({ embeds: [new EmbedBuilder().setTitle("📜 Rules").setDescription("Follow the server guidelines.").setColor(0xF1C40F)] }).catch(() => {});
...
client.on(Events.MessageCreate, async msg => {
  ...
  await msg.reply("📩 Notifications only. Contact an admin for assistance.");
});
