...
function getAcronym(company) { const words = company.trim().split(/\s+/); return words.length === 1 ? words[0].substring(0, 3).toUpperCase() : words.map(w => w[0].toUpperCase()).join(''); }
    ...
    if (interaction.isButton()) {
      ...
      if (interaction.customId.startsWith('approve_') || interaction.customId.startsWith('deny_')) {
        ...
        if (action === 'approve') {
          ...
          // TEXT CHANNEL - USING BITFIELDS TO PREVENT CRASH
          ...
          // VOICE CHANNEL - USING BITFIELDS TO PREVENT CRASH
          await interaction.guild.channels.create({
            name: `Voice Call`,
      ...
      if (interaction.customId === "back") { const dropdown = await buildDropdown(interaction.guild, bData.targets); return interaction.update({ content: "🎯 Select companies:", components: [new ActionRowBuilder().addComponents(dropdown)] }); }
      if (interaction.customId === "confirm") {
        ...
        for (const m of targetMembers.values()) {
          i++;
          try {
            await m.send({ embeds: [new EmbedBuilder().setColor(targets.includes("all") ? 0x2ecc71 : 0x3498db).setTitle("📢 Update").setDescription(messageContent).setFooter({ text: "Inter Molds, Inc." }).setTimestamp()] });
            success++;
          } catch (e) {}
